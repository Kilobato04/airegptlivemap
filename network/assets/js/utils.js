...
function createPopupContent(feature, sensorData) {
    ...
    if (APP_SETTINGS.activeStations.includes(name)) {
        ...
        if (sensorData && sensorData.dataIAS !== 'N/A') {
            ...
            html += '<a href="#" class="chart-link" onclick="toggleChartPanel(event, \'' + feature.properties.name + '\')" style="background-color: rgba(255,255,255,0.8); color: #333;">';
            ...
            // Contact Links con fondo semi-transparente para mejor visibilidad
            ...
            html += '<div style="margin-top: 4px; display: flex; align-items: center; gap: 8px; border: 2px solid #25D366; padding: 8px; border-radius: 8px; background-color: rgba(255, 255, 255, 0.9); cursor: pointer;">';
            ...
            // Footer info con fondo semi-transparente
            html += '<div style="margin-top: 8px; background-color: rgba(255,255,255,0.8); padding: 5px; border-radius: 5px;">';
            html += '<small style="color: #333;">Last updated: ' + new Date().toLocaleTimeString() + '</small><br>';
            html += '<small><a href="' + APP_SETTINGS.brandUrl + '" target="_blank" class="brand-text" style="color: #888;">smability.io</a></small>';
            html += '</div>';
