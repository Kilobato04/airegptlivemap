...
function createPopupContent(feature, sensorData) {
    ...
    if (APP_SETTINGS.activeStations.includes(name)) {
        ...
        if (sensorData && sensorData.dataIAS !== 'N/A') {
            ...
            // Historical Data Link - CON HOVER FADE
            html += '<div class="reading" style="margin-top: 4px; width: 100%;">';
            html += '<a href="#" class="chart-link" onclick="toggleChartPanel(event, \'' + feature.properties.name + '\')" style="background-color: transparent; color: #000; border: 2px solid #000; font-size: 14px; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'rgba(0,0,0,0.1)\'" onmouseout="this.style.backgroundColor=\'transparent\'">';
            ...
            html += '<div style="margin-top: 4px; display: flex; align-items: center; justify-content: center; gap: 8px; border: 2px solid #25D366; padding: 8px; border-radius: 8px; background-color: transparent; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'rgba(37,211,102,0.2)\'" onmouseout="this.style.backgroundColor=\'transparent\'">';
            ...
            // Footer info - TAMAÑOS HOMOLOGADOS, SIN NEGRITAS
            html += '<div style="margin-top: 8px; background-color: transparent; padding: 5px;">';
            html += '<small style="color: #000; font-size: 11px;">Last updated: ' + new Date().toLocaleTimeString() + '</small><br>';
            html += '<small><a href="' + APP_SETTINGS.brandUrl + '" target="_blank" class="brand-text" style="color: #000; font-size: 11px; text-decoration: none;">smability.io</a></small>';
            html += '</div>';
