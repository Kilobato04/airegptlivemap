...
function checkDependencies() {
    const required = [
        'MAPBOX_CONFIG', 'APP_SETTINGS', 'AQI_THRESHOLDS', 'MAP_LAYERS',
        'createLegendHTML', 'createPopupContent', 'fetchSensorData', 
        'updateMarkerColor', 'createMarkerElement', 'updateMarkerData'
    ];
...
// Solo inicializar si todas las dependencias están listas
if (!checkDependencies()) {
    console.error('Dependencies not ready, retrying in 100ms...');
    setTimeout(() => {
        if (checkDependencies()) {
            initializeMap();
        } else {
            console.error('Failed to load dependencies after retry');
        }
    }, 100);
} else {
    initializeMap();
